...
export default function ModalItem(props) {
    const { visible, item, onClose, addInCart, setItem } = props;
    ...
    const loadItem = (id) => {
        send('api/product/getbyid', 'GET', {id:id}, (json) => {
            setItem(json);
        }, token);
    }
    ...
    const sizes = item.sizes.map((obj) => {
        return (
            <TouchableOpacity style={styles.sizeField} onPress={() => loadItem(obj.id)}>
